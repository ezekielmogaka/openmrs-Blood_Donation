 ...
 */
package org.openmrs.module.blooddonation.api.db.hibernate;
...
/**
 * It is a default implementation of  {@link BloodDonationDAO}.
 */
public class HibernateblooddonationDAO implements BloodDonationDAO {
	...
	public List<BloodDonation> getRecordsByPatient(Patient patient) {
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(BloodDonation.class);
		criteria.add(Expression.eq("patient", patient));
		criteria.addOrder(Order.desc("bloodBankId"));
		return (List<BloodDonation>) criteria.list();
	}

	public BloodDonation getRecordById(int id){
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(BloodDonation.class);
		criteria.add(Expression.eq("bloodBankId",id));
		return (BloodDonation) criteria.uniqueResult();
	}
